...
describe('browserCapabilitiesProbe.supportedCodecs()', () => {
	it('returns an array', async () => {
		const result = await browserPlatform.capabilities.supportedCodecs?.();
		expect(Array.isArray(result)).toBe(true);
	});

	it('every entry in the returned list is a non-empty string', async () => {
		const result = await browserPlatform.capabilities.supportedCodecs?.() ?? [];
	...
	it('result is a subset of the probed codec set (no unknown entries)', async () => {
